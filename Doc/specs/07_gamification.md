...
(*※クエスト完了時に子供が感想を入力できるが、感想の有無はBaseTimeやBonusTimeの計算には影響しない*)
...
### 3. Reward Distribution (報酬の配分)
* 計算された `CalculatedEnergy` は、一度に全て付与されるのではなく、AIの解析完了時と親の承認時に分割される。
* **デフォルト配分比率**: AI判定時 : 親承認時 = 2 : 1
    * 例: 計算総量が45分の場合、AI解析完了時に30分が即座に付与され、親が承認したタイミングで残り15分（＋親からのボーナス）が付与される。
* **カスタマイズ**: この配分比率は親が設定画面から自由に変更可能。
