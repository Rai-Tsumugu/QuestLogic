# Week 5 Day 3: Final UI Polish（当日運用版）
...
## 本日のゴール
1. フォントサイズ、色、マージンが仕様書/デザイン案と一致している。
1. 誤字脱字がない。

## 本日の重点差分
- UI文言統一を観点表で確認し回帰を実施する。
...
### STEP-25.1 Consistency Check
- [ ] 実施操作: Consistency Check。実施内容: 全画面を見直し、スタイル崩れを修正。 / エラーメッセージの日本語がおかしくないか確認。 / 誰に見せても恥ずかしくないUI品質になっている。
- [ ] コマンド/操作: 実機で手順実行 + 結果をチェックシートへ記録
- [ ] 期待結果: Consistency Check に関する主導線がエラーなく完走し、異常系は意図した挙動になる。
- [ ] 証跡 (EVID-25.1): スクリーンショット、ログ抜粋、DB/Storage確認パスを記録する

### STEP-25.2 実機総合確認
- [ ] 実施操作: 本日の主導線を実機で通し、正常系と代表異常系を各1回確認する
- [ ] コマンド/操作: 実機で操作し、MetroログとFirebase Consoleを同時確認
- [ ] 期待結果: 完了判定に必要な証拠が揃い、再現性のある結果が得られる。
- [ ] 証跡 (EVID-25.2): 実機動画/スクリーンショット、ログ、判定メモ

### STEP-25.3 証跡整理とレビュー
- [ ] 実施操作: EVIDを整理し、未記録項目を埋めてレビュー観点を更新する
- [ ] コマンド/操作: チェックシート更新、スクリーンショット命名統一、ログ保存
- [ ] 期待結果: 完了判定に必要な証拠が揃い、再現性のある結果が得られる。
- [ ] 証跡 (EVID-25.3): 実機動画/スクリーンショット、ログ、判定メモ
