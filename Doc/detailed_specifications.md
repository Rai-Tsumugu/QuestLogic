...
本ドキュメントは、QuestLogicの各機能分野における詳細な仕様を定義するものです。
...
*   **Parent (親)**:
    ...
    *   認証: Social Login (Google)
*   **Child (子供)**:
    *   権限: クエスト（学習）の実行、撮影、報酬（時間）の消費。
    *   認証:
        *   **個人端末ログイン**: Social Login (Google)。
    **アカウント連携時**:
    *
...
### 3.1 ディレクトリ構造
Local Storage を利用し、アクセス権限を厳格に管理します。
...
*   **コールドデータ**: 3ヶ月経過後は、レポート用のサムネイル（低解像度）のみを残し、原本は削除（プライバシー保護とコスト削減）。
*   **削除ポリシー**: 退会時またはユーザーからの削除リクエスト時に削除。
...
*   **AIスコア係数**: AI評価(0.5 ~ 10倍)で変動。丁寧やるほど時間が増える。
...
3.  **アクション**:
    ...
    *   **強制ロック**: ゲーム・スマホの制限時間に関係なく強制的にデバイスをロック、解除できる。